...
void waiter_life(void){
	...
	while (sim->diningRoom->dead_philosophers!=sim->params->NUM_PHILOSOPHERS){
        ...
        if(n_phil!=0){
        	...
        	/* If it did operation of replenish food, didn't 'down' the semaphores, because
        	   Only needs to wait for dirty cutlery, not for pizza or spaghetti.
        	   So, it needs do 'down' now, because the philosophers will be awaken. */
        	if (semctl(sim->semid, S_ACCESS_WAITING_PHILOSOPHERS, GETVAL)>=1)
		        down(S_ALL_PHILISOPHERS_WAITING, 1);

		    /* Acordar filósofos na fila */
        	for (i=0; i<n_phil; i++){
