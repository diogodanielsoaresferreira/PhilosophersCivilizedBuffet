...
static void waiting_state(void);
...
void create(Simulation *s){
    ...
    sim->semid = semget(key+SHM_SIMULATION, 22+s->params->NUM_PHILOSOPHERS, 0600 | IPC_CREAT | IPC_EXCL);
    if (sim->semid == -1)
    {
...
void diningRoom_init(){
	...
	up(S_LOGGER, 1);
    up(S_ACCESS_WAITING_PHILOSOPHERS, 1);
}
...
void kill_phil(int id){
    int t = 0;
    down(S_ACCESS_WAITING_PHILOSOPHERS, 1);
    ...
    if (sim->diningRoom->num_phil_waiting==sim->params->NUM_PHILOSOPHERS-sim->diningRoom->dead_philosophers){
        up(S_ALL_PHILISOPHERS_WAITING, 1);
    }
...
/* Aumentar o nº de filósofos à espera.
Se todos estiverem à espera, fazer up do semáforo para sinalizar ao waiter que pode prosseguir com o pedido */
static void waiting_state(void){
    down(S_ACCESS_WAITING_PHILOSOPHERS, 1);
    down(S_KILL_ACCESS, 1);
    sim->diningRoom->num_phil_waiting+=1;
