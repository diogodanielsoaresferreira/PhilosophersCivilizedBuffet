...
static void go(Simulation* sim)
{
   ...
   pthread_create(&sim->waiter_thread, NULL, launch_waiter, NULL);
   
   int i;
   for(i=0; i<sim->params->NUM_PHILOSOPHERS;i++){
      pthread_create((pthread_t*)&sim->philosophers_thread[i],NULL,launch_phil, i);
   }
...
static void finish(Simulation* sim){
   ...
   int i;
   for(i=0;i<sim->params->NUM_PHILOSOPHERS;i++){
      pthread_join(sim->philosophers_thread[i],NULL);
   }
...
Simulation* initSimulation(Simulation* sim, Parameters* params)
{
   ...
   result->philosophers_thread = (pthread_t**)mem_alloc(sizeof(pthread_t*)*params->NUM_PHILOSOPHERS);
   for(i=0;i<params->NUM_PHILOSOPHERS;i++){
      result->philosophers_thread[i] = (pthread_t*)mem_alloc(sizeof(pthread_t));
   }

   // Waiter thread
   result->waiter_thread = (pthread_t*)mem_alloc(sizeof(pthread_t));
